...
class MyTcpHandler(socketserver.BaseRequestHandler):
   ...
   def handle(self): # 클라이언트가 접속시 클라이언트 주소 출력
      ...
      try:
         ...
         each2 = each[0].split(':')
         SensorName = each2[0] 
         SensingValue = each2[1]
         print(SiteName)
         print(SensorName)
         print(SensingValue)
         DB_Handle.my_insert(SiteName,SensorName,SensingValue)
         print("-----------------------")
         ret_msg = "OK"
         print(ret_msg)
         self.request.send(ret_msg.encode())
         #while msg:
